...
trait DefaultSchema extends {
  ...
  implicit val dateSchema: JsonSchema[LocalDate] = inlineInstance[LocalDate](
    Map(
      "type" -> "string",
      "format" -> "date").asJsonObject)

  implicit val dateTimeSchema: JsonSchema[LocalDateTime] =
    inlineInstance[LocalDateTime](
      Map(
        "type" -> "string",
        "format" -> "date-time").asJsonObject)

  implicit val uuidSchema: JsonSchema[UUID] =
    inlineInstance[UUID](Map("type" -> "string").asJsonObject)

  implicit val pathSchema: JsonSchema[Path] =
    inlineInstance[Path](Map("type" -> "string").asJsonObject)
  ...
  implicit def mapSchema[K, V](implicit kPattern: PatternProperty[K], vSchema: JsonSchema[V]): JsonSchema[Map[K, V]] =
    inlineInstance {
      JsonObject.fromMap(
        Map("patternProperties" -> JsonObject.singleton(
          k = kPattern.regex.toString,
          j = vSchema.asJsonRef).asJson))
    }
}
